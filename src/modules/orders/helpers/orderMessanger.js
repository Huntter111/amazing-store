...
export const sendOrderMessage = (order) => {
  ...
  const totalPrice = order.orderSummary?.priceTotalAmount?.totalPriceWithoutAdjastment;
  const totalPriceWithAdjastment = order.orderSummary?.priceTotalAmount?.totalPriceWithAdjastment;
  const adjastment = order.orderSummary?.adjastment?.adjastment * 100;

  const text = `<b>Нове замовлення №: ${order.orderNumber}</b> %0A Дата замовлення: ${order.orderDate} %0A Клієнт: ${order.userInfo.lastName} ${order.userInfo.firstName} %0A Телефон: ${order.userInfo.phone} %0A Email: ${order.userInfo.email} %0A <b>--------->Адреса доставки:</b> %0A Місто: ${order.address.city} %0A Вулиця: ${order.address.street} %0A Дім: ${order.address.house} %0A Квартира: ${order.address.apartment} %0A <b>--------->Замовлення:</b> %0A ${cartProductsMessage} <b>Всього до сплати: ${totalPrice} UAH</b> %0A <b>Всього до сплати з урахуванням знижки: ${totalPriceWithAdjastment} UAH</b> %0A <b>Знижка: ${adjastment} %</b>`;
